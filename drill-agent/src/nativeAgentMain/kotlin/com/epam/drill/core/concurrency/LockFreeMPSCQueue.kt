...
class LockFreeMPSCQueue<E : Any> {
    private val _cur =
        atomic(Core<E>(LockFreeMPSCQueueCore.INITIAL_CAPACITY))

    // Note: it is not atomic w.queue.t. remove operation (remove can transiently fail when isEmpty is false)
 ...
 * @suppress **This is unstable API and it is subject to change.**
 */
class LockFreeMPSCQueueCore<E : Any>(private val capacity: Int) {
...
class Symbol(@Suppress("UNUSED_PARAMETER") s: String)
