...
class PluginDispatcher(override val kodein: Kodein) : KodeinAware {
        ...
        app.routing {
            authenticate {
                patch<Routes.Api.Agent.UpdatePlugin> { ll ->
                    val config = call.receive<String>().trimIndent().replace("\n", "")
                    ...
                    if (agentManager.updateAgentPluginConfig(ll.agentId, pc)) {
                        sendPluginConfig(ll.agentId, ll.pluginId)
    ...
    private suspend fun sendPluginConfig(agentId: String, pluginId: String) {
        val config = agentManager[agentId]?.plugins?.find { it.id == pluginId }?.config ?: ""
