...
class AgentState(
    ...
    fun initialized() {
        ...
        val currentMethods = classBytes.mapValues { (className, bytes) ->
            ASMClassParser(bytes, className).parseToJavaMethods()
        }
        val prevMethods = prevData?.javaMethods?: mapOf()
        val diff = MethodsComparator().compareClasses(prevMethods, currentMethods)

        val prevAgentInfo = prevData?.agentInfo
        val changed = prevAgentInfo == agentInfo && diff.methodsChanged
        data = ClassesData(
            agentInfo = agentInfo,
